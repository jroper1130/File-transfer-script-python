...
#create list of docs in file 'a'
for root,dirs,files in os.walk(src):
    for file_name in files:
        #current time
        now = datetime.datetime.now()
        #diff var of 'now' and 'mod time' vars
        before = now - datetime.timedelta(hours=24)
        #join path to files in src
        files = os.path.join(src, file_name)
        #get mod time using getmtime,if modded in last 24 hrs then move date
